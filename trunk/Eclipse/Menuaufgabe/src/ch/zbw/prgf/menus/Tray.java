package ch.zbw.prgf.menus;

import javax.swing.JFrame;
import javax.swing.JMenu;
import javax.swing.JMenuBar;
import javax.swing.JMenuItem;

import java.awt.AWTException;
import java.awt.BorderLayout;
import java.awt.FlowLayout;
import java.awt.Menu;
import java.awt.MenuItem;
import java.awt.PopupMenu;
import java.awt.SystemTray;
import java.awt.TrayIcon;
import java.awt.TrayIcon.MessageType;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.awt.event.KeyEvent;
import java.io.IOException;

import javax.imageio.ImageIO;
import javax.swing.ImageIcon;
import javax.swing.JButton;
import javax.swing.JPanel;
import javax.swing.JPopupMenu;
import javax.swing.KeyStroke;
import javax.swing.border.EmptyBorder;

public class Tray implements ActionListener{
	//Menukomponenten definieren
	private Menu menu1, submenu;
	private MenuItem menuitem1, menuitem2, menuitem3, submenuitem1, submenuitem2;
	private PopupMenu popup;
	private TrayIcon trayIcon;
	private SystemTray tray;
	
	public Tray(){
		initComponents();
		bindListener();
		initTray();
	}
	
	private void initComponents() {
		//Menukomponenten initialisieren
		menu1 = new Menu("Menü 1");
		submenu = new Menu("Submenü 1");
		menuitem1 = new MenuItem("Menü Item 1");
		menuitem2 = new MenuItem("Menü Item 2");
		menuitem3 = new MenuItem("Menü Item 3");
		submenuitem1 = new MenuItem("SubMenü Item 1");
		submenuitem2 = new MenuItem("SubMenü Item 2");
		
		//Menü Struktur aufbauen
		menu1.add(menuitem1);
		menu1.add(menuitem2);
		menu1.add(menuitem3);
		menu1.addSeparator();
		menu1.add(submenu);
		submenu.add(submenuitem1);
		submenu.add(submenuitem2);
		
		//Ein Popup Menu wird erstellt
        popup = new PopupMenu();
        
      //Das Menu wird dem PopUp Menu zugefügt
        popup.add(menu1);
	}
	
	//Listeners einbinden
	private void bindListener(){
		menuitem1.addActionListener(this);
		menuitem2.addActionListener(this);
		menuitem3.addActionListener(this);
		submenuitem1.addActionListener(this);
		submenuitem2.addActionListener(this);
	}
	
	//actionPerformed für Menueinträge
	public void actionPerformed(ActionEvent event) {
		MenuItem clicked = (MenuItem)event.getSource();
		System.out.println("Menü " + clicked.getLabel() + " wurde ausgewählt.");
		//Balloon Message wird ausgegeben
		trayIcon.displayMessage("ALARM", "Ein Menueintrag wurde gklickt", MessageType.INFO);
	}
	
	//TrayIcon und Menu
	private void initTray(){
		//Ist ein SystemTray vom aktuellen System unterstützt?
        if (!SystemTray.isSupported()) {
            System.out.println("SystemTray wird nicht unterstützt");
            return;
        }
        
        //System TrayIcon definieren
        java.net.URL imageURL = this.getClass().getResource("/16x16.png");
		ImageIcon image = new ImageIcon(imageURL);
        		
        //Ein TrayIcon wird erstellt. Dem Konstruktor muss ein Bild übergeben werden
        trayIcon = new TrayIcon(image.getImage());
        //Der SystemTray wird angezogen
        tray = SystemTray.getSystemTray();
                
        //Das PopUpMenu wird als Menu für das Tray Icon definiert
        trayIcon.setPopupMenu(popup);
        //Tootltip definieren
        trayIcon.setToolTip("Dies ist ein tolles Tray Icon");
        
        //SysteTray.add wirft eine Exception
        try {
        	//Das TrayIcon wird dem SystemTray zugefügt
			tray.add(trayIcon);
		} catch (AWTException e) {
			System.out.println("TrayIcon konnte nicht zugefügt werden");
		}
	}
	
	public static void main(String[] args){
		Tray a1 = new Tray();
	}
	
	
}
